...
def GetTradeInformation(update: Update, trade: dict, balance: float) -> None:
    ...
    trade["PositionSize"] = (
        math.floor(((balance * trade["RiskFactor"]) / stopLossPips) / 10 * 100) / 100
    )
...
def CreateTable(
    trade: dict, balance: float, stopLossPips: int, takeProfitPips: int
) -> PrettyTable:
    ...
    table.add_row(
        [
            "Potential Loss",
            "$ {:,.2f}".format(round((trade["PositionSize"] * 10) * stopLossPips, 2)),
        ]
    )
...
def welcome(update: Update, context: CallbackContext) -> None:
    ...
    welcome_message = "Welcome to the FX Signal Copier Telegram Bot! 💻💸\n\nYou can use this bot to enter trades directly from Telegram and get a detailed look at your risk to reward ratio with profit, loss, and calculated lot size. You are able to change specific settings such as risk factor, and more from your personalized Python script and environment variables.\n\nUse the /help command to view instructions and example trades."
...
def help(update: Update, context: CallbackContext) -> None:
    ...
    update.effective_message.reply_text(
        trade_example + market_execution_example + limit_example + note
    )
...
def main() -> None:
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ...
            CALCULATE: [
                MessageHandler(Filters.text & ~Filters.command, CalculateTrade)
            ],
